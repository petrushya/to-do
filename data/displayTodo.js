...
class DisplayTodo extends DataCheck {
  ...
  projectsDaily() {
    ...
    btnRight.onclick = () => {
      ...
      btnRight.classList.toggle(
        "invisible",
        indexDayExist === datesExist.length - 1,
      );
      printPage(datesExist[indexDayExist]);
      btnRight.blur();
    };
      ...
      Object.keys(tasksDaily).forEach((key) => {
        if (day === key) {
          timeDay.textContent =
            day === todayDay
              ? "Today"
              : new Date(day).getDate() - new Date().getDate() === 1
                ? "Tomorrow"
                : new Date(day).getDate() - new Date().getDate() === -1
                  ? "Yesterday"
          ...
          new DataCheck().storageContent()[0].topicIds.forEach((mark) => {
            ...
            if (
              ...
              note.innerHTML =
                pas === 3
                  ? "End of <b>" +
                    content[mark].name.toUpperCase() +
                    "</b> on " +
                    new DateFormat(endDate).calendarDate() +
                    "!"
                  : "<b>" + content[mark].name.toUpperCase() + "</b>" +
                    " ends tomorrow!";
              button.onclick = () => {
          ...
          if (key === todayDay && !tasksDaily[todayDay].length) {
            ...
            message.innerText =
              "There are no planned tasks for today,\n" +
  ...
  tasksProject() {
    ...
    if (content[indexTopic].taskIds.length == 0) {
      ...
      message.innerText =
        "There are no tasks for your project yet." +
    ...
    const sortArr = [];
    content[indexTopic].taskIds.forEach((mark) =>
      sortArr.push(content[mark].id),
    );
    sortArr.sort();
    ...
    indexArr.forEach((mark) => {
      ...
      if (dayArg !== new DateFormat(content[mark].startTime).calendarDate()) {
        let ifToday =
          new DateFormat(content[mark].startTime).dayString() ===
  ...
  topicsTodo() {
    ...
    content[0].topicIds.forEach((mark) =>
      sortNameProject.push(content[mark].id),
    );
    ...
    sortNameProject.forEach((mark) => {
      ...
      pasTopic.textContent =
        content[indexTopic].pas === 3
          ? "3 days ahead"
          : content[indexTopic].pas === 1
            ? "a day ahead"
            : "Never";
      infoTopic.textContent =
        content[indexTopic].info !== "" ? content[indexTopic].info : "Non";
  ...
  startPage() {
    ...
    message.innerText =
      "There are no projects for your to-do list yet." +
      "\nWant to create one?";
  ...
  #taskBlock(index) {
    ...
    const btnDeleteTask = !this.dayTime
      ? document.createElement("button")
      : false;
    const btnProjList = this.dayTime ? document.createElement("button") : false;
    ...
    const spanInfo =
      content[index].info && !this.dayTime
    ...
    if (content[index].completed) checkTask.setAttribute("checked", "");
    ...
    startTask.className =
      content[index].completed === false ? checkClass(index) : "";
    startTask.setAttribute(
      "datetime",
      new DateFormat(content[index].startTime).dateString(),
    );
    startTask.textContent = new DateFormat(
      content[index].startTime,
    ).timesOfDay();
    pasTask.innerHTML =
      content[index].pas === "Low"
        ? "&#x272d;"
    ...
    function checkClass(index) {
      ...
      if (
        new DateFormat(+content[index].id).dayString() <
      ...
      if (checkTaskArr.length > 0) {
        if (
          checkTaskArr.includes(content[index].id) &&
  ...
  #pageHeader() {
    ...
    btnDaily.style.display =
      hasDisplay && Object.keys(new DataCheck().idToIndexTaskObj()).length > 0
        ? ""
        : "none";
    btnProjects.onclick = () => {

...
class DialogForm extends DataCheck{
  ...
  dialogForm(){
    ...
    const task = document.querySelector('#formFieldTask') ?
      document.querySelector('#formFieldTask') : false;
    ...
    if(this.type === 'topicMake') {
      const topicsIdExist = [];
      if(topicsIndexes.length) topicsIndexes.forEach(value => {
      ...
      name.addEventListener('input', (e) => {
        ...
        let checkValue = e.target.value.trim().includes(' ') ?
          e.target.value.trim().replace(' ', '-') : e.target.value.trim();
    ...
    if(this.type.startsWith('task')) {
      ...
      const dateDefault = date.defaultValue ?
        Date.parse(date.defaultValue) : 0;
      if(topicsIndexes.length) topicsIndexes.forEach(topic => {
    ...
    btnConfirm.addEventListener('click', (e) => {
      if(checkValueTopic || !name.validity.valid) {
        ...
        if(this.type.startsWith('topic')) {
          ...
          const taskData =
            [argName, date.value, pas.value, info.value, task.value];
  ...
  #dialogPattern(){
    ...
    const fieldName = this.type === 'taskMake' ?
    ...
    legend.textContent = this.type === 'topicMake' ? 'Create a Project':
      this.type === 'topicRevise' ? 'Revise a Project' :
      this.type === 'taskMake' || this.type === 'taskAdd' ?
      'Create a Task' : 'Revise a Task';
    labelName.textContent = this.type === 'topicMake' ?
    ...
    if(this.idTopic) {
      ...
      if(this.type === 'topicRevise') {
        ...
        fieldPas.value = storage[indexTopic].pas === 3 ?
        '3 days ahead' : storage[indexTopic].pas === 1 ?
        'a day ahead' : 'Never';

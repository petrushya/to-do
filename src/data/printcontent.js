...
export class printContent {
  ...
  get pageContent(){
    const sitePageTitle = document.querySelector('head title');
    ...
    if(Object.keys(JSON.parse(localStorage.getItem('projList'))).length === 0){
      ...
      sitePageTitle.textContent = 'daily to-do list';
      ...
      mainTitle.textContent = sitePageTitle.textContent = 'topics list';
      ...
      if(JSON.parse(localStorage.getItem('todoList'))[this.pageLink].length === 0){
        ...
        mainTitle.textContent = sitePageTitle.textContent = this.pageLink;
  ...
  #getarticle(){
    ...
    Object.keys(JSON.parse(localStorage.getItem('todoList'))).forEach(key => {
      ...
      titlePart.className = 'navlink';
      titlePart.tabindex = 0;
      titlePart.textContent = key;
      titlePart.onclick = () => new printContent(key).pageContent;

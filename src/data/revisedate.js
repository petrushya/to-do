export class reviseDate {
  ...
  get hourMinute(){
    ...
    const inputHour = (hour < 10) ? `0${hour}` : `${hour}`;
    const minute = new Date(this.oneDate).getMinutes();
    const inputMinute = (minute < 10) ? `0${minute}`: `${minute}`;
    return `${inputHour}:${inputMinute}`;
  }
  get fullDate(){
    const month = new Date(this.oneDate).getMonth();
    const inputMonth = (month < 10) ? `0${month+1}` : `${month+1}`;
    const date = new Date(this.oneDate).getDate();
    const inputDate = (date < 10) ? `0${date}`: `${date}`;
